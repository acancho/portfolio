...
<?php
  ...
  <script>
    document.getElementById('color_mode').addEventListener('change', function() {
      if (this.checked) {
        window.location.href = 'ingles/nuevo_foto.php';
      }
    });
  </script>
  ...
  <div class="container text-center">
    ...
    if (move_uploaded_file($archivo_ruta_temporal, "img/" . $archivo_nombre)) {
      ...
      echo "An error occurred while uploading the file. Could not be saved.";
    }
    ?>
    <?php
    ...
    $sql = "INSERT INTO fotos (foto) VALUES ('$archivo_nombre')";

    // Comprobamos si el registro se ha guardado correctamente y en caso de error lo mostramos.
    if (mysqli_query($mysqli, $sql)) {
      echo "<div class='row'>
                <div class='col'>
                    <h2>Registro guardado exitosamente</h2><a class='btn btn-primary' href='crud.php'>Volver atrás</a> 
                </div>";
      echo "<br>";
    } else {
