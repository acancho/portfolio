...
<div class="container">
  ...
  <ol class="breadcrumb">
    <li class="breadcrumb-item "><a href="trabajos.php" class="text-dark link-offset-2 link-underline link-underline-opacity-0">Trabajos</a></li>

    <li class="breadcrumb-item active"><a href="" class="text-dark link-offset-2 link-underline link-underline-opacity-0">
    <?php
include 'includes/conexion.php';
$id_trabajo = $_GET['id'];

$sql = "SELECT * FROM `trabajos` WHERE id = $id_trabajo";
    $resultado = mysqli_query($mysqli, $sql);

    // Verificar si se encontraron resultados
    if(mysqli_num_rows($resultado) > 0) {
        $fila = mysqli_fetch_assoc($resultado);
        // Mostrar los detalles del trabajo
...
if(isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Consulta SQL para obtener los detalles del trabajo específico
    $sql = "SELECT * FROM `trabajos` WHERE id = $id_trabajo";
    $resultado = mysqli_query($mysqli, $sql);
    ...
    if(mysqli_num_rows($resultado) > 0) {
        ...
        echo "    <img src='img/" . $fila['imagen'] . "' alt='Foto' class='img-fluid' style='max-width: 300px;'>";
        echo "  </div>";
        echo "  <div class='card-body'>";
        echo "    <h1 class='card-title'>" . $fila['nombre'] . "</h1>";
        echo "    <p class='card-text'><strong>Material:</strong> " . $fila['material'] . "</p>";
        echo "    <p class='card-text'><strong>Tipo:</strong> " . $fila['tipo'] . "</p>";
        echo "    <p class='card-text'><strong>Localización:</strong> " . $fila['localizacion'] . "</p>";
        echo "    <h6 class='mt-4'><strong> Descripción </strong> </h6>";
        echo "    <p>" . $fila['descripcion'] . "</p>";
        echo "  </div>";
        echo "  <div class='card-footer text-muted'>";
        echo "    <a href='trabajos.php' class='btn btn-outline-info'>Volver a Trabajos</a>";
        echo "  </div>";
        echo "</div>";
    } else {
        echo "No se encontraron detalles para este trabajo.";
    }
} else {
